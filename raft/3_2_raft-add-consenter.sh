...
sleep 4
COMPOSE_PROJECT_NAME=${ORDERER_NAME}.${ORDERER_DOMAIN} EXECUTE_BY_ORDERER=1 runCLI "container-scripts/orderer/raft-add-consenter.sh $NEWCONSENTER_NAME $NEWCONSENTER_DOMAIN ${NEWCONSENTER_PORT} ${NEWCONSENTER_WWW_PORT}"
sleep 4
COMPOSE_PROJECT_NAME=${ORDERER_NAME}.${DOMAIN}  EXECUTE_BY_ORDERER=1 runCLI "container-scripts/orderer/raft-add-endpoint.sh ${NEWCONSENTER_NAME} ${NEWCONSENTER_DOMAIN} ${NEWCONSENTER_PORT}"
sleep 4
